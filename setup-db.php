...
if ($action === 'install') {
    ...
    // 3. Désactiver les vérifications de clés étrangères pour l'installation
    try {
        $pdo->exec("SET FOREIGN_KEY_CHECKS = 0");
        $successes[] = "Vérifications FK désactivées temporairement";

        // Vérifier que c'est bien désactivé
        $fkCheck = $pdo->query("SELECT @@FOREIGN_KEY_CHECKS")->fetchColumn();
        $successes[] = "FK_CHECKS = " . $fkCheck;
    } catch (PDOException $e) {
        $errors[] = "Impossible de désactiver FK_CHECKS: " . $e->getMessage();
    }

    // 4. Lire et adapter le schéma (remplacer 'deco' par DB_NAME)
    ...
    $successes[] = "Vérifications FK réactivées";
